...
- **时间线**：每人最近 6 场峡谷对局读取 SGP DETAILS（`{区}_{gameId}/DETAILS`），同一局只取一次并缓存 400 局。
  得出 15 分钟前被敌方打野参与击杀次数（Akari 的「好抓」口径）和 10 分钟对位经济/补刀差。
- **战力值**（0–100，50 为平均）= 50 + 胜率项（向 50% 收缩 5 场，权重 60）+ Akari Score 项（±12）+ 对线项（每 50 经济 1 分，±10）。
  样本不足 5 场不计算。
- **标签**：队内比中位数高/低 ≥6 → 我方「上等马 / 下等马」、敌方「硬骨头 / 软柿子」；
  同位置战力差 ≥10 → 「对位优势 / 劣势」；好抓 / 非常好抓 / 难抓；对线强 / 弱（±350 经济）。
